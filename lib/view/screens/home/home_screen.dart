...
class HomeScreen extends StatelessWidget {
  ...
  HomeScreen({@required this.org, this.reload});
  static Future<void> loadData(bool reload, int orgId) async {
    Get.find<BannerController>().getBannerList(reload);
    Get.find<CategoryController>().getCategoryList(reload);
    Get.find<RestaurantController>()
        .getPopularRestaurantList(reload, 'all', false, orgId);
    Get.find<CampaignController>().getItemCampaignList(reload);

    Get.find<RestaurantController>()
        .getLatestRestaurantList(reload, 'all', false, orgId);

    Get.find<RestaurantController>().getRestaurantList('1', reload, orgId);
  ...
  init() async {
    await App.init();
    App.sharedPreferences.setInt('orgId', org.orgId);
  }
        ...
        child: RefreshIndicator(
          onRefresh: () async {
            ...
            await Get.find<RestaurantController>()
                .getPopularRestaurantList(true, 'all', false, org.orgId);
            await Get.find<CampaignController>().getItemCampaignList(true);
            // await Get.find<ProductController>()
            //     .getPopularProductList(true, 'all', false);
            await Get.find<RestaurantController>()
                .getLatestRestaurantList(true, 'all', false, org.orgId);
            // await Get.find<ProductController>()
            //     .getReviewedProductList(true, 'all', false);
            await Get.find<RestaurantController>()
                .getRestaurantList('1', true, org.orgId);
            if (Get.find<AuthController>().isLoggedIn()) {
